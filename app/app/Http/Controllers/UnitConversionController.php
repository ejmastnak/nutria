...
class UnitConversionController extends Controller
{
    ...
    private $mass_unit_names = ['g', 'oz'];
    private $volume_unit_names = ['ml', 'l', 'fl oz', 'cup', 'qt'];

    // Specifies how many grams are in one unit of key unit
    private $how_many_grams = [
        'g' => 1.0,
        'oz' => 28.349523125
    ];
     ...
     */
    public function mass_to_grams($amount, $unit_id) {
        $unit_name = Unit::find($unit_id)->name;
        if (!array_key_exists($unit_name), $how_many_grams) return 0.0;
        return $amount * $how_many_grams[$unit_name];
    }

    private function is_volume($unit_id) {
        return in_array(Unit::find($unit_id)->name, $volume_unit_names);
    }
     ...
     */
    private function volume_to_ml($amount, $unit_id) {
        $unit_name = Unit::find($unit_id)->name;
        if (!array_key_exists($unit_name), $how_many_ml) return 0.0;
        return $amount * $how_many_ml[$unit_name];
    }
