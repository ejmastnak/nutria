        ...
        DB::transaction(function () use ($data, $userId, &$foodList, $convertToGramsService) {
            ...
            foreach (data['food_list_ingredients'] as $idx=>$fli) {
                $fli = FoodListIngredient::create([
                    ...
                    'amount' => $fli['amount'],
                    'unit_id' => $fli['unit_id'],
                    'mass_in_grams' => $convertToGramsService->convertToGrams($fli['amount'], $fli['unit_id'], $fli['ingredient_id']),
                    'seq_num' => $idx,
                ]);
            ...
            foreach (data['food_list_meals'] as $idx=>$flm) {
                $flm = FoodListMeal::create([
                    ...
                    'amount' => $flm['amount'],
                    'unit_id' => $flm['unit_id'],
                    'mass_in_grams' => $convertToGramsService->convertToGrams($flm['amount'], $flm['unit_id'], $flm['meal_id']),
                    'seq_num' => $idx,
                ]);
