    ...
    <table v-if="body_weight_records.length" class="text-sm sm:text-base text-left text-gray-500">
      ...
      <tbody>
        <tr
          ...
          class="border-b"
        >
          <td scope="row" class="px-8 py-4 font-medium text-gray-900 text-right">
            <button
              type="button"
              @click="logBodyWeightDialogRef.open(body_weight_record)"
              class="mx-auto p-px rounded-md hover:underline focus:outline-none focus:ring-2 focus:ring-blue-700 whitespace-nowrap"
            >
          ...
          <td>
            <div class="flex items-center px-8">
              ...
              <button
                type="button"
                @click="idToDelete = body_weight_record.id; deleteDialog.open()"
